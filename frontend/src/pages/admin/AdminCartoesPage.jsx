...
function AdminCartoesPage() {
  ...
  async function handleDeleteCard(cartao) {
    ...
    const statusAtual = String(cartao.status || '').toUpperCase();
    if (statusAtual === 'ATIVO') {
      setError('Somente cartões desativados/cancelados podem ser excluídos.');
      return;
    }

    setDialogState({
      title: 'Excluir cartão',
      message: `Excluir permanentemente o cartão ${cartao.identificacao}?`,
      ...
      onConfirm: async () => {
        try {
          if (!cartao.localOnly) {
            await deleteAdminCard(cartao.id);
          }
          setCartoes((current) => current.filter((item) => item.id !== cartao.id));
          setError('');
          setDialogState(null);
        } catch {
  ...
  async function handleCreateCard() {
    ...
    try {
      const response = await createAdminCard({
        clienteNome: newCard.clienteNome,
        email,
        identificacao: newCard.identificacao,
        tipo: newCard.tipo,
      ...
      const card = response.cartao;
      const normalizedCard = {
        ...card,
        clienteNome: card?.usuario_nome || newCard.clienteNome,
        identificacao: card?.nome_cartao || newCard.identificacao,
        tipo: card?.tipo_servico || newCard.tipo,
        criadoEm: card?.data_entrada || newCard.criadoEm,
        validade: card?.data_saida || newCard.validade,
        status: card?.status || newCard.status,
        email: card?.usuario_email || email,
      };

      setCartoes((current) => [normalizedCard, ...current]);

      setCreatedCredentials({
        email: response?.credenciais?.email || email,
        senha: response?.credenciais?.senhaTemporaria || '',
        usuario: response?.cartao?.usuario_id,
      });
      ...
      const backendMessage = e?.response?.data?.error || e?.message || 'Falha ao criar conta do cliente.';
      setCreatedCredentials({
        email,
        senha: '',
        error: backendMessage,
      });
      setError(`Não foi possível registrar cartão. ${backendMessage}`);
    }
  }
    ...
    <Layout>
      <div className="page-container">
                            ...
                            {cartao.status === 'ATIVO' ? (
                              ...
                              <>
                                ...
                                <button
                                  className="link-button link-button-danger"
                                  type="button"
                                  onClick={() => handleDeleteCard(cartao)}
                                >
                                  Excluir
                                </button>
                              </>
        ...
        <AppDialog
          ...
          cancelLabel={dialogState?.cancelLabel || 'Cancelar'}
