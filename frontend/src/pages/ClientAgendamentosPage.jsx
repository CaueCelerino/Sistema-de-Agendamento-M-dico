...
function ClientAgendamentosPage() {
  ...
  useEffect(() => {
    const singleProfessional = selectedAvailability?.porProfissionais?.length === 1
      ? selectedAvailability.porProfissionais[0]
      : null;

    if (singleProfessional) {
      setSelectedProfessional(Number(singleProfessional.funcionario_id));
      return;
    }

    const firstProfessionalId = selectedAvailability?.porProfissionais?.[0]?.funcionario_id;
    if (firstProfessionalId != null) {
      const currentIsValid = selectedAvailability.porProfissionais.some(
        (item) => Number(item.funcionario_id) === Number(selectedProfessional)
      );
      if (!currentIsValid) {
        setSelectedProfessional(Number(firstProfessionalId));
      }
      return;
    }

    if (selectedAvailability && selectedAvailability.porProfissionais?.length === 0) {
  ...
  async function handleRequestAppointment() {
    if (!selectedHour || !selectedDate || !selectedProfessional) {
      setError('Selecione um profissional específico para continuar.');
      return;
    }
    try {
                  ...
                  <>
                    ...
                    {selectedAvailability.porProfissionais && selectedAvailability.porProfissionais.length === 1 && (
                      <div style={{ marginBottom: '1rem' }}>
                        <p style={{ margin: 0 }}>
                          <strong>Profissional:</strong> {selectedAvailability.porProfissionais[0].funcionario}
                          {selectedAvailability.porProfissionais[0].especialidade ? ` (${selectedAvailability.porProfissionais[0].especialidade})` : ''}
                        </p>
                      </div>
                    )}
                    ...
                    {selectedAvailability.porProfissionais && selectedAvailability.porProfissionais.length > 1 && (
                      <div style={{ marginBottom: '1rem' }}>
                        ...
                        <select
                          value={selectedProfessional || selectedAvailability.porProfissionais[0]?.funcionario_id || ''}
                          onChange={(e) => {
