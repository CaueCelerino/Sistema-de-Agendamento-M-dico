...
router.get('/disponibilidades', (req, res, next) => {
  ...
  db.all(
    `SELECT a.id, a.funcionario_id, a.data_agenda AS data, a.horario_inicio AS horario, a.horario_fim, a.disponivel, f.nome AS funcionario, f.especialidade
     ...
     WHERE a.data_agenda LIKE ? AND a.disponivel = 1 AND a.funcionario_id IS NOT NULL AND f.id IS NOT NULL
...
router.post('/disponibilidades', (req, res, next) => {
  ...
  const resolvedFuncionarioId = Number(funcionario_id);
  if (!Number.isFinite(resolvedFuncionarioId) || resolvedFuncionarioId <= 0) {
    return res.status(400).json({ error: 'funcionario_id é obrigatório para cadastrar disponibilidade' });
  }

  db.get('SELECT id, nome FROM funcionarios WHERE id = ? AND ativo = 1', [resolvedFuncionarioId], (funcErr, funcRow) => {
    if (funcErr) return next(funcErr);

    if (!funcRow) {
      return res.status(400).json({ error: 'funcionário inválido ou inativo' });
    }
    ...
    db.serialize(() => {
      ...
      stmt.finalize((finalizeErr) => {
        if (finalizeErr) return next(finalizeErr);
        return res.status(201).json({ data, horarios, profissional: profissional || funcRow.nome });
      });
    });
  });
});
...
router.get('/disponibilidades/:data/horarios-profissionais', (req, res, next) => {
  ...
  db.all(
    `SELECT a.funcionario_id, f.nome AS funcionario, f.especialidade, a.horario_inicio AS horario
     FROM agendas a
     INNER JOIN funcionarios f ON f.id = a.funcionario_id
     WHERE a.data_agenda = ? AND a.disponivel = 1 AND a.funcionario_id IS NOT NULL AND f.id IS NOT NULL
