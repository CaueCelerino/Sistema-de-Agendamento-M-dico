...
CREATE TABLE IF NOT EXISTS relatorios (
    ...
    tipo TEXT NOT NULL CHECK(tipo IN ('RELATORIO', 'AVISO', 'COMUNICADO')),
    visibilidade TEXT NOT NULL DEFAULT 'ADMIN' CHECK(visibilidade IN ('ADMIN', 'CLIENTES')),
    autor TEXT NOT NULL,
    publicado_em DATE DEFAULT (DATE('now')),
    ativo BOOLEAN DEFAULT 1,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX IF NOT EXISTS idx_device_tokens_usuario ON device_tokens(usuario_id);
CREATE INDEX IF NOT EXISTS idx_device_tokens_ativo ON device_tokens(ativo);
CREATE INDEX IF NOT EXISTS idx_relatorios_visibilidade ON relatorios(visibilidade);
CREATE INDEX IF NOT EXISTS idx_relatorios_publicado_em ON relatorios(publicado_em);
