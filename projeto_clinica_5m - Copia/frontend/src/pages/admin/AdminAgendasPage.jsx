...
function AdminAgendasPage() {
  ...
  async function handleSalvarEdicoesHorarios() {
    ...
    try {
      ...
      // 2. Adicionar novos horários
      const novosPorMedico = Object.entries(selectedHoursByFuncionario)
        .filter(([funcionario_id, horarios]) => {
          const temRemovidos = horariosParaRemover[funcionario_id] || [];
          const horariosAntigos = selectedAvailability.porProfissionais
            .find((p) => p.funcionario_id === Number(funcionario_id))
            ?.horarios || [];
          
          return horarios.some((h) => !horariosAntigos.includes(h));
        })
      ...
      if (novosPorMedico.length > 0) {
        await saveAvailability({ data: selectedDate, disponibilidadesPorMedico: novosPorMedico });
      }
                ...
                <div className="form-grid">
                  ...
                  <div>
                    ...
                    {selectedAvailability ? (
                      <p style={{ fontSize: '0.9rem', marginBottom: '0.5rem' }}>Edite os horários dos profissionais abaixo.</p>
                    ) : null}
                        ...
                        funcionarios.map((f) => {
                          ...
                          return (
                            <button
                              ...
                              style={{
                                opacity: selectedAvailability !== undefined ? 0.5 : 1,
                                cursor: selectedAvailability !== undefined ? 'not-allowed' : 'pointer',
                              }}
                            >
                              {f.nome}
                            </button>
                    ...
                    {selectedAvailability && selectedAvailability.porProfissionais && selectedAvailability.porProfissionais.length > 0 ? (
                      <div>
                        ...
                        <small style={{ display: 'block', marginTop: '1rem', fontStyle: 'italic' }}>
                          Dica: Clique em um horário existente para marcá-lo para remoção (ficará com linha riscada). Use o input abaixo para adicionar novos horários.
                        </small>
                      </div>
                    ) : (
                      <>
